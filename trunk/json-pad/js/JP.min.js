...
Array.prototype.in_array=function(needle){for(var i=0;i<this.length;i++)if(this[i]===needle)return true;return false};var UpdateApplication={appUpdater:new runtime.air.update.ApplicationUpdaterUI,configXml:"app:/update-config.xml",getApplicationInfo:function(){var xmlString=air.NativeApplication.nativeApplication.applicationDescriptor;var appXml=new DOMParser;var xmlObject=appXml.parseFromString(xmlString,"text/xml");var root=xmlObject.getElementsByTagName("application")[0];return[root.getElementsByTagName("version")[0].firstChild.data,root.getElementsByTagName("name")[0].firstChild.data]},getApplicationVersion:function(){return this.getApplicationInfo()[0]},
getApplicationName:function(){return this.getApplicationInfo()[1]},checkUpdate:function(){UpdateApplication.appUpdater.checkNow()},onInitialization:function(){UpdateApplication.checkUpdate()},onError:function(event){alert(event.toString())}};var airApplicationExit=function(){var exitingEvent=new air.Event(air.Event.EXITING,false,true);air.NativeApplication.nativeApplication.dispatchEvent(exitingEvent);if(!exitingEvent.isDefaultPrevented())air.NativeApplication.nativeApplication.exit()};
var JSONpadAir={prefsFile:null,prefsXML:null,stream:null,settings:null,init:function(){window.nativeWindow.addEventListener(air.Event.CLOSING,JSONpadAir.windowClosingHandler);this.prefsFile=air.File.applicationStorageDirectory;this.prefsFile=this.prefsFile.resolvePath("settings.xml");this.readXML();UpdateApplication.appUpdater.configurationFile=new air.File(UpdateApplication.configXml);UpdateApplication.appUpdater.addEventListener(air.ErrorEvent.ERROR,UpdateApplication.onError);UpdateApplication.appUpdater.addEventListener("initialized",
UpdateApplication.onInitialization);UpdateApplication.appUpdater.initialize()},readXML:function(){this.stream=new air.FileStream;if(this.prefsFile.exists){this.stream.open(this.prefsFile,air.FileMode.READ);this.processXMLData()}else this.saveData(null);window.nativeWindow.visible=true},processXMLData:function(){this.prefsXML=this.stream.readUTFBytes(this.stream.bytesAvailable);this.stream.close();var domParser=new DOMParser;this.prefsXML=domParser.parseFromString(this.prefsXML,"text/xml");var syntaxHl=
this.prefsXML.getElementsByTagName("syntaxHighlighting")[0];this.settings={syntax_hl:syntaxHl.getAttribute("showCodeMirror")}},saveData:function(data){this.createXMLData(data);this.writeXMLData()},createXMLData:function(data){if(data==null)data={syntax_hl:true};var cr=air.File.lineEnding;this.prefsXML="<?xml version='1.0' encoding='utf-8'?>"+cr+"<preferences>"+cr+"    <syntaxHighlighting showCodeMirror='"+(data.syntax_hl?"true":"false")+"' />"+cr+"    <saveDate>"+(new Date).toString()+"</saveDate>"+
cr+"</preferences>"},writeXMLData:function(){this.stream=new air.FileStream;this.stream.open(this.prefsFile,air.FileMode.WRITE);this.stream.writeUTFBytes(this.prefsXML);this.stream.close()},windowClosingHandler:function(event){debug.trace("start windowClosingHandler");JSONpadAir.saveData({syntax_hl:Ext.getCmp("JPviewPort").findByType("jp_main_top")[0].getTopToolbar().findByType("jp_main_top_iconbar")[0].btnGroup_others.switchHighlighting.pressed});event.preventDefault();nativeWindow.removeEventListener("closing",
JSONpadAir.windowClosingHandler);JP.util.air.exit()}};var JSLINT=function(){var adsafe_id,adsafe_may,adsafe_went,anonname,approved,atrule={media:true,"font-face":true,page:true},bang={"<":true,"<=":true,"==":true,"===":true,"!==":true,"!=":true,">":true,">=":true,"+":true,"-":true,"*":true,"/":true,"%":true},banned={arguments:true,callee:true,caller:true,constructor:true,eval:true,prototype:true,stack:true,unwatch:true,valueOf:true,watch:true},boolOptions={adsafe:true,bitwise:true,browser:true,cap:true,css:true,debug:true,devel:true,eqeqeq:true,es5:true,
...
*/
console={log:air.Introspector?air.Introspector.Console.log:function(){}};Ext.BLANK_IMAGE_URL="js/extjs/resources/images/default/s.gif";Ext.ns("JP");Ext.onReady(function(){Ext.QuickTips.init();var viewport=new Ext.Viewport({layout:"border",id:"JPviewPort",items:[{xtype:"jp_main_top"},{xtype:"jp_main_center"},{xtype:"jp_main_bottom"}]})});air.Introspector.Console.info();JP.util={exit:function(){if(Ext.isAir)JP.util.air.exit();else JP.util.web.exit()},copyToClipboard:function(string){if(Ext.isAir)JP.util.air.copyToClipboard(string);else JP.util.web.copyToClipboard(string)},pasteFromClipboard:function(){if(Ext.isAir)return JP.util.air.pasteFromClipboard();else return JP.util.web.pasteFromClipboard()},validateJson:function(json,clear){if(!json.errorObject){debug.trace("clear?"+clear);if(clear)JP.util.clearCodeMirrorStatus();return true}else{JP.util.setCodeMirrorStatus({text:json.errorObject.length+
...
json.errorObject[i].evi.trim()+"</li>"}if(JP.ErrorWindow.Action.window.isVisible())JP.ErrorWindow.Action.window.update('<ul style="">'+msg+"</ul>");var errorContainer=Ext.get(Ext.query(".x-status-error-detail-link"));errorContainer.on("click",function(e,t,o){var cmp=Ext.getCmp(t.id);var pos=cmp.getPosition();if(JP.ErrorWindow.Action.me==null)JP.ErrorWindow.Action.me=this;JP.ErrorWindow.Action.openWindow();JP.ErrorWindow.Action.window.update('<ul style="">'+msg+"</ul>");JP.ErrorWindow.Action.window.setPosition(pos[0]+
...
var rootNode=tree.getRootNode();var jsonString="";var lb="";var spacer="";var pt=":";if(editKeyForm.formUnsaved){var sm=tree.getSelectionModel();var selectedNode=sm.getSelectedNode();editKeyForm.confirmUnsaved(selectedNode);return false}if(!compress){lb="\n";spacer="\t";pt=" : "}var buildJSONString=function(node,jsonString,lvl,compress){lvl++;var tab="";for(var i=1;i<lvl;i++)tab+=spacer;var collapseNode=false;if(node.isExpandable()&&!node.isExpanded()){node.expand();collapseNode=true}node.eachChild(function(child){if(lvl==
1)debug.trace(child.text);if(child.hasChildNodes()){if(child.attributes.type=="array"&&node.attributes.type!="array")jsonString+=tab+spacer+'"'+child.attributes.text+'"'+pt+"["+lb;else{jsonString+=tab+spacer;if(node.attributes.type!="array")jsonString+='"'+child.attributes.text+'"'+pt;jsonString+=(child.attributes.type=="array"?"[":"{")+lb}jsonString=tab+buildJSONString(child,jsonString,lvl,compress);jsonString+=tab+spacer+(child.attributes.type=="array"?"]":"}")}else{var nodeValue=child.attributes.value;
if(child.attributes.type=="string")nodeValue='"'+nodeValue+'"';jsonString+=tab+spacer;if(node.attributes.type=="array")jsonString+=nodeValue;else jsonString+='"'+child.attributes.text+'"'+pt+nodeValue}jsonString+=(!child.isLast()?",":"")+lb});if(collapseNode){node.collapse();collapseNode=false}return jsonString};if(rootNode.attributes.type=="array")jsonString+="["+lb;else jsonString+="{"+lb;jsonString=buildJSONString(rootNode,jsonString,0,compress);if(rootNode.attributes.isArray)jsonString+="]"+lb;
else jsonString+="}"+lb;stringInputField.setValue(jsonString.trim());JP.util.setJPStatus({text:"Tree built to JSON string",iconCls:"x-status-valid",clear:true},"left");return true},compressJsonString:function(){var stringInputField=this.findParentByType("viewport").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0];var stringCompressed=jsmin("",stringInputField.getValue(),3);stringInputField.setValue(stringCompressed.trim());JP.util.setJPStatus({text:"Compressed JSON string succesfully",
iconCls:"x-status-valid",clear:true},"left")},formatJsonString:function(){var stringInputField=this.findParentByType("viewport").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0];var json=JP.util.parseJson(stringInputField.getValue(),true);if(JP.util.validateJson(json,true)){stringInputField.setValue(JSON.stringify(json,null,"  "));JP.util.setJPStatus({text:"Formatted JSON string succesfully",iconCls:"x-status-valid",clear:true},"left")}},validateJsonString:function(){var stringInputField=
this.findParentByType("viewport").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0];if(stringInputField.getValue().trim()=="")JP.util.setJPStatus({text:"Empty JSON string",iconCls:"x-status-error",clear:true},"left");else{var json=JP.util.parseJson(stringInputField.getValue(),true);if(JP.util.validateJson(json,false))JP.util.setCodeMirrorStatus({text:"JSON string is valid",iconCls:"x-status-valid",clear:true})}},switchHighlighting:function(btn,pressed){var stringInput=this.findParentByType("viewport").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0];
if(pressed==true)stringInput.showCodeMirror();else stringInput.hideCodeMirror()},insertExample:function(){this.findParentByType("viewport").findByType("jp_main_center_stringForm")[0].findByType("ux-codemirror")[0].setValue(this.example)}};JP.MainTop.mainBar=Ext.extend(Ext.Toolbar,{initComponent:function(){this.items=[{text:"File",ref:"file",menu:{ref:"menu",items:[{text:"Quit",statusBarTip:"Quit the application",ref:"quit"}]}},{text:"Edit",ref:"edit",menu:{ref:"menu",items:[{text:"XML2JSON Converter",ref:"xml2json",statusBarTip:"Convert XML data to a JSON string and insert it in the textarea"},"-",{text:"Copy JSON string",ref:"copy",statusBarTip:"Copy JSON string to clipboard"},{text:"Paste JSON string",ref:"paste",statusBarTip:"Paste JSON string from cliboard"}]}},
...
JP.MainCenter.Action.tree={addKeyNode:function(){JP.MainCenter.Action.tree.addNode("key",this)},addObjectNode:function(){JP.MainCenter.Action.tree.addNode("object",this)},addArrayNode:function(){JP.MainCenter.Action.tree.addNode("array",this)},addNode:function(nodeType,tree){var sm=tree.getSelectionModel();var selectedNode=sm.getSelectedNode();var parentNode=null;if(selectedNode==null)selectedNode=tree.getRootNode();if(selectedNode.attributes.type=="object"||selectedNode.attributes.type=="array")parentNode=
selectedNode;else parentNode=selectedNode.parentNode;if(!parentNode.hasChildNodes()&&parentNode.getDepth()==0){var root=JP.util.getJsonTreeDefaultRootNode("object",[]);tree.setRootNode(root);tree.getLoader().load(tree.root);parentNode=tree.getRootNode()}var newChildConfig=null;switch(nodeType){case "key":newChildConfig={leaf:true,expandable:false,value:"",type:"string"};break;case "object":case "array":newChildConfig={leaf:false,expandable:true,type:nodeType.toLowerCase(),iconCls:"ico_"+nodeType.toLowerCase()};
break}newChildConfig.id=Ext.id();if(parentNode.attributes.type=="array"&&(nodeType=="object"||nodeType=="array"))newChildConfig.text="[object "+nodeType.toFirstUpperCase()+"]";else newChildConfig.text="New"+nodeType.toFirstUpperCase();var newNode=new Ext.tree.TreeNode(newChildConfig);parentNode.insertBefore(newNode,selectedNode.nextSibling);if(parentNode.isExpandable()&&!parentNode.isExpanded())parentNode.expand();sm.select(newNode)},duplicateNode:function(){var tree=Ext.getCmp("JPviewPort").findByType("jp_main_center_jsonTree")[0];
var sm=tree.getSelectionModel();var selectedNode=sm.getSelectedNode();var parentNode=selectedNode.parentNode;if(selectedNode!=null)if(selectedNode.getDepth()!=0){var duplicateTreeObj=function(node,obj){node.eachChild(function(child){var newChild=new Object;newChild.id=Ext.id();if(child.hasChildNodes()){newChild.iconCls=child.attributes.iconCls;newChild.expandable=true;newChild.leaf=false;newChild.children=duplicateTreeObj(child,[])}else{newChild.expandable=false;newChild.leaf=true;newChild.value=
child.attributes.value}newChild.text=child.attributes.text;newChild.type=child.attributes.type;obj.push(newChild)});return obj};var newNode=new Ext.tree.TreeNode;if(selectedNode.hasChildNodes()){var children=duplicateTreeObj(selectedNode,[]);newNode.appendChild(children)}else newNode.attributes.value=selectedNode.attributes.value;newNode.setId(Ext.id());newNode.attributes.iconCls=selectedNode.attributes.iconCls;newNode.attributes.expandable=selectedNode.attributes.expandable;newNode.attributes.leaf=
selectedNode.attributes.leaf;newNode.setText(selectedNode.attributes.text+" Copy");newNode.attributes.type=selectedNode.attributes.type;parentNode.insertBefore(newNode,selectedNode.nextSibling);sm.select(newNode)}},deleteNode:function(){var tree=Ext.getCmp("JPviewPort").findByType("jp_main_center_jsonTree")[0];var sm=tree.getSelectionModel();var selectedNode=sm.getSelectedNode();var previousSibling=selectedNode.previousSibling;var parentNode=selectedNode.parentNode;if(selectedNode)if(selectedNode.getDepth()!=
0)selectedNode.remove();if(previousSibling)sm.select(previousSibling);else sm.select(parentNode)},setTreePath:function(node,tree){var treePath=node.getPath("text");treePath=treePath.substr(1,treePath.length).split("/");var treePathIds=node.getPath("id");treePathIds=treePathIds.substr(1,treePathIds.length).split("/");var rowNr=null;var pathStr="";var count=0;for(rowNr in treePath)if(rowNr!="remove"&&rowNr!="in_array"){if(count==0)pathStr+="/ ";else if(node.id==treePathIds[rowNr])pathStr+=treePath[rowNr];
else pathStr+='<a href="#" class="tree-path-link" nodelinkid="'+treePathIds[rowNr]+'">'+treePath[rowNr]+"</a> / ";count++}var mainBottom=tree.findParentByType("viewport").findByType("jp_main_bottom")[0];mainBottom.updateContent(pathStr);Ext.get(Ext.query(".tree-path-link")).on("click",function(e,el,obj){var tree=this;var selectNode=tree.getNodeById(el.getAttribute("nodelinkid"));var selectionModel=tree.getSelectionModel();selectionModel.select(selectNode)},tree)}};JP.MainCenter.stringForm=Ext.extend(Ext.FormPanel,{title:"",region:"north",layout:"fit",split:true,height:200,id:"JPsouth",initComponent:function(){var me=this;this.items=[{xtype:"ux-codemirror",codeMirrorPath:"js/extern/CodeMirror",language:"js",hideLabel:true,name:"jsonString",initCallBackFn:function(cm){me.setWidth(me.getWidth()-1);if(JSONpadAir.settings.syntax_hl=="false"){Ext.getCmp("JPviewPort").findByType("jp_main_top")[0].getTopToolbar().findByType("jp_main_top_iconbar")[0].btnGroup_others.switchHighlighting.toggle(false,
true);cm.hideCodeMirror()}}}];this.bbar={xtype:"statusbar",id:"JPstringInputStatusBar",statusAlign:"left",defaultText:"",text:"",iconCls:""};JP.MainCenter.stringForm.superclass.initComponent.call(this)}});Ext.reg("jp_main_center_stringForm",JP.MainCenter.stringForm);JP.MainCenter.jsonTree=Ext.extend(Ext.tree.TreePanel,{title:"",region:"west",split:true,width:300,border:true,enableDD:true,autoScroll:true,ddGroup:"nodeDragAndDrop",treeContextMenu:null,initComponent:function(){var sm=this.getSelectionModel();var me=this;this.plugins=[new NodeMouseoverPlugin,new NodeMouseoutPlugin];this.tbar={xtype:"jp_main_center_jsonTree_toolbar"};this.root={text:"JSON",draggable:false,leaf:true,ref:"rnode",type:"object"};JP.MainCenter.jsonTree.superclass.initComponent.call(this);
...
me.addListener("contextmenu",function(node,event){var selectionModel=me.getSelectionModel();selectionModel.select(node);me.treeContextMenu.showAt(event.getXY());event.stopEvent()},me);sm.addListener("beforeselect",function(sel,n,o){debug.trace("blub2");var editKeyForm=this.findParentByType("viewport").findByType("jp_main_center_ediTreeForm_tabs_edit")[0];var topBar=this.getTopToolbar();if(editKeyForm.formUnsaved){editKeyForm.confirmUnsaved(n);return false}if(n.getDepth()==0){editKeyForm.disable();
topBar.duplicateNode.disable();topBar.deleteNode.disable();this.treeContextMenu.duplicateNode.disable();this.treeContextMenu.deleteNode.disable()}else{topBar.duplicateNode.enable();topBar.deleteNode.enable();this.treeContextMenu.duplicateNode.enable();this.treeContextMenu.deleteNode.enable();if(n.attributes.leaf==true){editKeyForm.keyForm.jsonkey.setValue(n.attributes.text);editKeyForm.keyForm.jsonvalue.setValue(n.attributes.value);editKeyForm.keyForm.isnull.setValue(n.attributes.type=="null");if(n.parentNode!=
null&&n.parentNode.attributes.type=="array")editKeyForm.keyForm.jsonkey.disable();else editKeyForm.keyForm.jsonkey.enable();if(n.attributes.type=="null")editKeyForm.keyForm.jsonvalue.disable();else editKeyForm.keyForm.jsonvalue.enable();editKeyForm.enableKeyForm()}else{editKeyForm.enableObjectForm();editKeyForm.objectForm.jsonIndex.setValue(n.attributes.text)}}JP.MainCenter.Action.tree.setTreePath(n,this);return true},me)}});Ext.reg("jp_main_center_jsonTree",JP.MainCenter.jsonTree);JP.MainCenter.jsonTree.nodeToolbar=Ext.extend(Ext.Toolbar,{ref:"treetools",initComponent:function(){this.items=[{xtype:"button",iconCls:"icon_tree_addKey",tooltip:"Add a key/object/array",ref:"addNode",menu:{ref:"menu",items:[{xtype:"menuitem",text:"Add key",ref:"addKey"},{xtype:"tbseparator"},{xtype:"menuitem",text:"Add object",ref:"addObject"},{xtype:"menuitem",text:"Add array",ref:"addArray"}]}},{xtype:"button",iconCls:"icon_tree_duplicate",tooltip:"Duplicate selected node",disabled:true,ref:"duplicateNode"},
"->",{xtype:"button",iconCls:"icon_tree_delete",tooltip:"Delete selected node",disabled:true,ref:"deleteNode"}];JP.MainCenter.jsonTree.nodeToolbar.superclass.initComponent.call(this);var tree=this.findParentByType("jp_main_center_jsonTree");this.addNode.menu.addKey.setHandler(JP.MainCenter.Action.tree.addKeyNode,tree);this.addNode.menu.addObject.setHandler(JP.MainCenter.Action.tree.addObjectNode,tree);this.addNode.menu.addArray.setHandler(JP.MainCenter.Action.tree.addArrayNode,tree);this.duplicateNode.setHandler(JP.MainCenter.Action.tree.duplicateNode,
tree);this.deleteNode.setHandler(JP.MainCenter.Action.tree.deleteNode,tree)}});Ext.reg("jp_main_center_jsonTree_toolbar",JP.MainCenter.jsonTree.nodeToolbar);JP.MainCenter.jsonTree.contextMenu=Ext.extend(Ext.menu.Menu,{initComponent:function(){this.items=[{text:"Add",iconCls:"icon_tree_addKey",ref:"addNode",menu:{ref:"menu",items:[{text:"Add Key",ref:"addKey"},"-",{text:"Add Object",ref:"addObject"},{text:"Add Array",ref:"addArray"}]}},{text:"Duplicate",iconCls:"icon_tree_duplicate",ref:"duplicateNode"},"-",{text:"Delete",iconCls:"icon_tree_delete",ref:"deleteNode"}];JP.MainCenter.jsonTree.contextMenu.superclass.initComponent.call(this);var tree=Ext.getCmp("JPviewPort").findByType("jp_main_center_jsonTree")[0];
...
var activeTabXtype=getActiveTab.getXType();switch(activeTabXtype){case "jp_main_center_ediTreeForm_tabs_edit":this.findByType("jp_main_center_ediTreeForm_tabs_edit")[0].saveForm(getActiveTab);break}},me)}});Ext.reg("jp_main_center_ediTreeForm",JP.MainCenter.ediTreeForm);JP.MainCenter.ediTreeForm.tabs=Ext.extend(Ext.TabPanel,{activeTab:0,resizeTabs:true,border:false,initComponent:function(){this.items=[{xtype:"jp_main_center_ediTreeForm_tabs_edit"}];JP.MainCenter.ediTreeForm.tabs.superclass.initComponent.call(this)}});Ext.reg("jp_main_center_ediTreeForm_tabs",JP.MainCenter.ediTreeForm.tabs);JP.MainCenter.ediTreeForm.tabs.edit=Ext.extend(Ext.Panel,{title:"Edit",layout:"anchor",frame:false,border:false,disabled:true,formUnsaved:false,initComponent:function(){var me=this;this.items=[{xtype:"jp_main_center_ediTreeForm_tabs_edit_key"},{xtype:"jp_main_center_ediTreeForm_tabs_edit_object"}];JP.MainCenter.ediTreeForm.tabs.edit.superclass.initComponent.call(this);me.addListener("afterrender",function(){this.disable()},me);me.addListener("disable",function(){this.keyForm.jsonkey.setValue("");
...
1);JP.XmlWindow.Action.setXmlData()}}];JP.XmlWindow.xmlForm.superclass.initComponent.call(this);var tb=this.getTopToolbar();tb.btnGroup_clipboard.copy.setHandler(JP.XmlWindow.Action.tbar.copyXmlStringToClipboard,this);tb.btnGroup_clipboard.paste.setHandler(JP.XmlWindow.Action.tbar.pasteXmlStringFromClipboard,this);tb.btnGroup_others.switchHighlighting.on("toggle",JP.XmlWindow.Action.tbar.switchHighlighting,this)}});Ext.reg("jp_xmlwindow_xmlform",JP.XmlWindow.xmlForm);JP.AboutWindow=Ext.extend(Ext.Window,{layout:"fit",width:345,height:290,closeAction:"hide",plain:true,modal:true,resizable:false,draggable:false,title:"About JSONpad",id:"JP_aboutWindow",initComponent:function(){var me=this;var text='<span style="font-size: 11px;">'+"<p>This project started by Christopher S&ouml;llinger and is a Non-Profit free software by the {2}.<br />It is open source, so feel "+"free to download the latest source code at {3}.<br />&nbsp;<br />"+"If you have any problems you have different methods to give it back to me:</p>"+
"<p>- Write it to {4}<br />"+"-&nbsp;Or you can even post an issue to {5}<br />"+"-&nbsp;Or just get it contact with me:<br />"+'&nbsp;&nbsp;<a href="#" id="mailto:zerogiven@gmail.com" class="link-to-browser">zerogiven@gmail.com</a><br />'+'&nbsp;&nbsp;<a href="#" id="http://www.twitter.com/JSONpad/" class="link-to-browser">http://www.twitter.com/JSONpad</a>'+"</p>&nbsp;<br />"+"<p>If you want have more informations or news about JSONpad, feel free to visit the {6} or the {7}.</p>&nbsp;<br />"+"</span>"+
'<span style="font-size: 9px;">'+"JSONpad Version Number: {0}<br />"+"ExtJS Version Number: {1}<br />&nbsp;<br />"+"</span>";var jsonPadVersion=UpdateApplication.getApplicationVersion();var extjsVersion=Ext.version;var licenseLink='<a href="#" id="http://www.jsonpad.co.cc/new_bsd_license.txt" class="link-to-browser">New BSD License</a>';var googleCodeLink='<a href="#" id="http://code.google.com/p/json-pad/" class="link-to-browser">Google Code</a>';var googleGroupsLink='<a href="#" id="http://groups.google.com/group/jsonpad" class="link-to-browser">Google Groups</a>';
var homepageLink='<a href="#" id="http://www.jsonpad.co.cc/" class="link-to-browser">homepage</a>';var blogLink='<a href="#" id="http://jsonpad.blogspot.com/" class="link-to-browser">blog</a>';var aboutHtml=String.format(text,jsonPadVersion,extjsVersion,licenseLink,googleCodeLink,googleGroupsLink,googleCodeLink,homepageLink,blogLink);this.items=[new Ext.Panel({html:'<div style="padding: 5px;">'+aboutHtml+"</div>"})];JP.AboutWindow.superclass.initComponent.call(this);this.addListener("afterrender",
function(){Ext.select("a.link-to-browser").on("click",function(e,el){var href=el.id;openUrl(href);return false})})}});Ext.reg("jp_aboutwindow",JP.AboutWindow);JP.AboutWindow.Action={me:null,window:null,openWindow:function(){if(!JP.AboutWindow.Action.window)JP.AboutWindow.Action.window=new JP.AboutWindow;JP.AboutWindow.Action.window.show()}};JP.ErrorWindow=Ext.extend(Ext.Window,{layout:"fit",closeAction:"hide",plain:true,x:0,y:0,baseCls:"x-tip",bodyBorder:false,draggable:false,resizable:false,html:"Dummytext",id:"JP_errorWindow",initComponent:function(){var me=this;JP.ErrorWindow.superclass.initComponent.call(this)}});Ext.reg("jp_errorwindow",JP.ErrorWindow);JP.ErrorWindow.Action={me:null,window:null,init:function(){if(!JP.ErrorWindow.Action.window)JP.ErrorWindow.Action.window=new JP.ErrorWindow},openWindow:function(){if(!JP.ErrorWindow.Action.window)JP.ErrorWindow.Action.init();JP.ErrorWindow.Action.window.show()}};