var JsonTreeFunctions = {
	...
	selectionModel.select(jsonTree.getRootNode());
    },
	...
	var selectionModel = jsonTree.getSelectionModel();
	debug.trace("-"+nodeId);
	var node = jsonTree.getNodeById(nodeId);
	node.remove();
...
var JSONpad_JsonTree = {
	...
	Ext.getCmp("JsonTree").on("afterRender", function (treepanel) {
	    ...
	    var btnDuplicate = Ext.getCmp("btn_menu_tree_duplicate");
		...
		notifyDrop: function(dd, e, data) {
		    (function() {
			JsonTreeFunctions.duplicateNode();
		    }).defer(50);
		    return true;
		}
	    });
	...
	Ext.getCmp("JsonTree").addListener("startdrag", function (tree, node, event) {
	    debug.trace("STARTDRAG");
	    Ext.getCmp("JsonTree_tbar_add").disable();
	});

	Ext.getCmp("JsonTree").addListener("enddrag", function (tree, node, event) {
	    debug.trace("ENDDRAG");
	    Ext.getCmp("JsonTree_tbar_add").enable();
